-- Fix exchange_convert_simple to use profiles.saldo_moeda as primary RIOZ balance source
...
DECLARE
  ...
  v_result JSON;
BEGIN
  -- Get current BRL balance from balances table
  SELECT brl_balance 
  INTO v_brl_balance
  FROM balances 
  WHERE user_id = p_user_id;
  
  -- Get current RIOZ balance from profiles table (primary source)
  SELECT saldo_moeda::NUMERIC
  INTO v_rioz_balance
  FROM profiles
  WHERE id = p_user_id;
  
  -- If no balance record exists in balances table, create one
  IF v_brl_balance IS NULL THEN
    INSERT INTO balances (user_id, brl_balance, rioz_balance)
    VALUES (p_user_id, 1000, COALESCE(v_rioz_balance, 0));
    v_brl_balance := 1000;
  END IF;
  
  -- If no profile exists (shouldn't happen), return error
  IF v_rioz_balance IS NULL THEN
  ...
  ELSIF p_operation = 'sell_rioz' THEN
    -- Check if user has enough RIOZ in profiles table
  ...
  -- Update profiles table (primary RIOZ balance)
  UPDATE profiles 
