...
-- 2. Criar cron job para fechar mercados automaticamente (executa a cada hora)
SELECT cron.schedule(
  'close-expired-markets-hourly',
  '0 * * * *', -- A cada hora no minuto 0
  $$
  SELECT net.http_post(
    url := 'https://ufvupwnhpqeexadmqajk.supabase.co/functions/v1/close-expired-markets',
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVmdnVwd25ocHFlZXhhZG1xYWprIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg1ODg3NjYsImV4cCI6MjA3NDE2NDc2Nn0.iL0h7rYwsmhx8XhSOhbiu4gwFw4YewbNPkn4WUdtBWU"}'::jsonb,
    body := '{"scheduled": true}'::jsonb
  );
  $$
);

-- 3. Inserir mercados de exemplo para demonstração
...
-- Inserir configuração inicial do sistema se não existir
